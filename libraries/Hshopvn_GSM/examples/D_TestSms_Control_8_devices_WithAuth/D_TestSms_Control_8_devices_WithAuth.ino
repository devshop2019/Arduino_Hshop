...
void GSM_Ready() {
  if (HGSM.getDataGSM() != "") {
    ...
    if(HGSM.checkData("84913909190")){
      passOK = true;
    }
    for (unsigned char count_for = 0; count_for < 8; count_for++) {
      if (HGSM.checkData(KeyWordOF[count_for]) && passOK) {            // if Key word (turn off) match
        digitalWrite(Relay_pin[count_for], !Relay_active);   // Turn off relay
        Serial.println("I turn off relay: " + String(count_for));
        passOK = false;
      } else if (passOK && HGSM.checkData(KeyWordON[count_for])) {     // if Key word (turn on) match
