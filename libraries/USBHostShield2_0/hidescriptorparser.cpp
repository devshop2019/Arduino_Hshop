...
uint8_t myCode[MAX_LEN_CODE];
uint8_t countItemInmyCode = 0;
uint8_t countItem = 0;
bool complete_bit = 0;
unsigned long timewait = 0;
bool timewait_disable = true;
...
const char * const ReportDescParserBase::genDesktopTitles0[] PROGMEM = {    //kxn2
        pstrUsagePointer,
        pstrUsageMouse,
        pstrUsageJoystick,
        pstrUsageGamePad,
...
//                Serial.println("KXN test 8: " + String(countItem));// Da thay ngay phia truoc doi tuong *KXN*
//                PrintByteValue(result.dwResult);
                if(countItem == 9){
//                    Serial.print("Gia Tri Tim Duoc ");
//                    Serial.print(result.dwResult,HEX);
//                    Serial.print(" tai ");
//                    Serial.println(countItemInmyCode);
                    if(result.dwResult != 0){
                        myCode[countItemInmyCode] = (uint8_t)result.dwResult;
                        countItemInmyCode++;
//                        timewait = millis();
                    }
                    timewait = millis();
...
//                if(countItemInmyCode == 14){
//                   complete_bit = true;
//                }
                countItem++;
//                Serial.println("KXN test 7: ");// Da thay ngay phia sau doi tuong *KXN*
        }
        E_Notify(PSTR("\r\n"), 0x80);
}

void UniversalReportParser::Parse(HID *hid, bool is_rpt_id, uint8_t len, uint8_t *buf) {
//    Serial.println("KXN test 6: ");// Da thay doi tuong *KXN*
    timewait_disable = false;
    timewait = millis();
    countItem = 0;
