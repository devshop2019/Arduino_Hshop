...
extern bool complete_bit;
extern unsigned long timewait;
extern bool timewait_disable;
...
uint8_t* MAX3421e< SPI_SS, INTR >::bytesRd(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
        ...
        printf("\r\n");
//        Serial.println("KXN test 4_6: ");// Tim Khong thay!
#else
                *data_p++ = SPDR;
        }
#endif
...
//                    Serial.println("KXN test 4_5: ");// Da tim thay thu nam!
                        break;
                }
        }
        return ( i);
...
//        Serial.println("KXN test 4_4: ");// Da tim thay thu tu!
...
int8_t MAX3421e< SPI_SS, INTR >::Init(int mseconds) {
        ...
        busprobe(); //check if anything is connected
//        Serial.println("KXN test 4_4: ");// Khong thay!
...
//                        Serial.println("KXN test 4_4: ");// Da tim thay thu hai!
                        break;
                case( bmKSTATUS):
                        if((regRd(rMODE) & bmLOWSPEED) == 0) {
                                regWr(rMODE, MODE_LS_HOST); //start low-speed host
...
uint8_t MAX3421e< SPI_SS, INTR >::Task(void) {
        ...
        if(pinvalue == 0) {
//            Serial.println("KXN test 4_3: ");// Quet lien tuc
                if(timewait_disable == false){
                    if(millis() - timewait >= 150){
                        complete_bit = true;
                        timewait_disable = true;
                    }
                }
                rcode = IntHandler();
        }
...
uint8_t MAX3421e< SPI_SS, INTR >::IntHandler() {
        ...
        if(HIRQ & bmCONDETIRQ) {
//            Serial.println("KXN test 4_3: ");// Da tim thay thu ba!
